...
class GetUniverisitiesByEmailAgent(ScAgent):
    ...
    def RunImpl(self, evt: ScEventParams) -> ScResult:
        ...
        if self.module.ctx.HelperCheckEdge(
                self.keynodes['action_get_universities_by_email'],
                self.main_node,
            ...
            try:
                ...
                emailIterator = self.ctx.Iterator5(
                    ScType.Unknown,
                    ScType.EdgeDCommon,
                    node,
                    ScType.EdgeAccessConstPosPerm,
                    self.keynodes['nrel_email']
                )
                while emailIterator.Next():
                    for i in range(5):
                        self.add_nodes_to_answer(answerNode, [emailIterator.Get(i)])
