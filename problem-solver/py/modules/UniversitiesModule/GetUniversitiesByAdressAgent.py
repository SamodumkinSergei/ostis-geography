...
class GetUniversitiesByAdressAgent(ScAgent):
    ...
    def RunImpl(self, evt: ScEventParams) -> ScResult:
        ...
        if self.module.ctx.HelperCheckEdge(
                self.keynodes['action_get_universities_by_adress'],
                self.main_node,
            ...
            try:
                ...
                adressIterator = self.ctx.Iterator5(
                    ScType.Unknown,
                    ScType.EdgeDCommon,
                    node,
                    ScType.EdgeAccessConstPosPerm,
                    self.keynodes['nrel_adress']
                )
                while adressIterator.Next():
                    for i in range(5):
                        self.add_nodes_to_answer(answerNode, [adressIterator.Get(i)])
