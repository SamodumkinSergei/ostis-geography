...
class GetUniversitiesByFacultiesAgent(ScAgent):
    ...
    def RunImpl(self, evt: ScEventParams) -> ScResult:
        ...
        if self.module.ctx.HelperCheckEdge(
                self.keynodes['action_get_univerisites_by_faculties'],
                self.main_node,
            ...
            try:
                ...
                facultyIterator = self.ctx.Iterator5(
                    ScType.Unknown,
                    ScType.EdgeDCommon,
                    node,
                    ScType.EdgeAccessConstPosPerm,
                    self.keynodes['nrel_number_of_faculties']
                )
                while facultyIterator.Next():
                    for i in range(5):
                        self.add_nodes_to_answer(answerNode, [facultyIterator.Get(i)])
