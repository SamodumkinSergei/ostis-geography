...
class GetUniversitiesByPassingGradeAgent(ScAgent):
    ...
    def RunImpl(self, evt: ScEventParams) -> ScResult:
        ...
        if self.module.ctx.HelperCheckEdge(
                self.keynodes['action_get_univerisites_by_passing_grade'],
                self.main_node,
            ...
            try:
                ...
                gradeIterator = self.ctx.Iterator5(
                    ScType.Unknown,
                    ScType.EdgeDCommon,
                    node,
                    ScType.EdgeAccessConstPosPerm,
                    self.keynodes['nrel_average_passing_grade']
                )
                while gradeIterator.Next():
                    for i in range(5):
                        self.add_nodes_to_answer(answerNode, [gradeIterator.Get(i)])
