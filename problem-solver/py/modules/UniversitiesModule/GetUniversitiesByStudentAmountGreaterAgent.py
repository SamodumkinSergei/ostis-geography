...
class GetUniversitiesByStudentAmountGreaterAgent(ScAgent):
    ...
    def RunImpl(self, evt: ScEventParams) -> ScResult:
        ...
        if self.module.ctx.HelperCheckEdge(
                self.keynodes['action_get_universities_by_student_amount_greater'],
                self.main_node,
            ...
            try:
                ...
                universityIterator = self.ctx.Iterator3(
                    seld.keynodes['concept_university'],
                    ScType.EdgeAccessConstPosPerm,
                    ScType.Unknown,
                )
                while universityIterator.Next():
                    university = universityIterator.Get(2)
                    NumberIterator= self.ctx.Iterator5(
                        university,
                        ScType.EdgeDCommon,
                        ScType.Unknown,
                        ScType.EdgeAccessConstPosPerm,
                        self.keynodes['nrel_number_of_students']
                    )
                    ...
                    if number > givenNumber:
                        for i in range(5):
                            self.add_nodes_to_answer(answerNode, [universityIterator.Get(i)])
