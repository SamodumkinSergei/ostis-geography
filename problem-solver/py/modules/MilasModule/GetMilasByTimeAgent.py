...
class GetMilasByTimeAgent(ScAgentClassic):
    def __init__(self):
        super().__init__("GetMilasByTimeAgent") # your agent name
    ...
    def RunImpl(self, evt: ScResult) -> ScResult:
        ...
        status = ScResult.Ok
        self.log.debug("GetMilasByTimeAgent starts")
        if self.module.ctx.HelperCheckEdge(
                self.keynodes['action_get_milas_by_time'],
                self.main_node,
            ...
            try:
                ...
                self.log.debug("GetMilasByTimeAgent get arguments")

                timeNode = self.get_action_argument(self.main_node, 'rrel_1')
                
                answerNode = self.ctx.CreateNode(ScType.NodeConstStruct)
                self.add_nodes_to_answer(answerNode, [timeNode])

                milasWithTime = self.get_milas_with_time()
                time = float(self.get_main_idtf(timeNode))
                
                results = []
                for mila, milaTime in milasWithTime.items():
                    if time == milaTime:
                        results.append(mila)

                for mila in results:
                    self.log.debug("GetMilasByTimeAgent get answer")
                    self.add_mila_to_answer(mila, answerNode)

                self.finish_agent(self.main_node, answerNode)
                self.log.debug("GetMilasByTimeAgent ends")
    ...
    def add_mila_to_answer(self, mila, answer):
        ...
        template.TripleWithRelation(
            ...
            ScType.LinkVar >> '_time',
            ScType.EdgeAccessVarPosPerm >> "_arc_2",
            self.keynodes['nrel_open_time'],
        )
        search_result = self.ctx.HelperSearchTemplate(template)
        ...
        if search_result.Size():
            self.add_nodes_to_answer(answer,
             [
                ...
                search_result[0]['_time']
            ]
            )

    def get_milas_with_time(self):
        template = ScTemplate()
        template.Triple(
        ...
        if search_result.Size():
            for i in range(search_result.Size()):
                shops_with_time[search_result[i]['_shop']] = float(self.ctx.GetLinkContent(search_result[i]['_time']).AsString())
        return shops_with_time
