...
class SearchUniversityByRegionAgent(ScAgentClassic):
    def __init__(self):
        super().__init__("action_search_university_by_region")
    ...
    def on_event(self, class_node: ScAddr, edge: ScAddr, action_node: ScAddr) -> ScResult:
        ...
        status = ScResult.OK
        _logger.debug("SearchUniversityByRegionAgent starts")

        try:
            _logger.debug("SearchUniversityByRegionAgent get arguments")
            ...
            self.find_university(node, answer_node)
            self.finish_agent(action_node, answer_node)

            _logger.debug("SearchUniversityByRegionAgent ends")
    ...
    def find_university(self, university_region: ScAddr, answer_node: ScAddr) -> None:
        university_template = ScTemplate()
        university_template.triple_with_relation(
            [sc_types.UNKNOWN, "_university"],
            sc_types.EDGE_D_COMMON_VAR,
            [university_region, "_region_node"],
            sc_types.EDGE_ACCESS_VAR_POS_PERM,
            self._keynodes["nrel_region"]
        )
        _logger.debug("1st passed")
        university_template.triple(
            self._keynodes['concept_university'],
            [sc_types.EDGE_ACCESS_VAR_POS_PERM, "_edge_to_university"],
            "_university"
        )
        _logger.debug("2st passed")
        result = client.template_search(university_template)
        wrap_in_set(answer_node, self._keynodes['concept_university'])

        for item in result:
            _logger.debug("SearchUniversityByRegionAgent get answer: " +
