...
class SearchSmallerPharmacyChainsAgent(ScAgentClassic):
    def __init__(self):
        super().__init__("action_search_smaller_pharmacy_chains")
    ...
    def find_bigger_chains(self, main_chain: ScAddr, answer_node: ScAddr) -> None:
        ...
        for item in chains_result:
            ...
            # в ответ добавляются только те сети, которые меньше начальной
            if (chain_length < main_chain_length):
                _logger.debug(f"{get_system_idtf(chain)} is smaller... adding it to answer")
