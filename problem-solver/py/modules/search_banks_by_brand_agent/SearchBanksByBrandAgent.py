...
class SearchBanksByBrandAgent(ScAgentClassic):
    def __init__(self):
        super().__init__("action_search_banks_by_brand")
    ...
    def on_event(self, class_node: ScAddr, edge: ScAddr, action_node: ScAddr) -> ScResult:
        ...
        status = ScResult.OK
        _logger.debug("SearchBanksByBrandAgent starts")

        try:
            _logger.debug("SearchBanksByBrandAgent get arguments")
            ...
            self.find_banks_by_brand(node, answer_node)
            self.finish_agent(action_node, answer_node)

            _logger.debug("SearchBanksByBrandAgent ends")
    ...
    def find_banks_by_brand(self, bank_brand: ScAddr, answer_node: ScAddr) -> None:
        bank_template = ScTemplate()
        bank_template.triple_with_relation(
            [sc_types.UNKNOWN, "_bank"],
            sc_types.EDGE_D_COMMON_VAR,
            [bank_brand, "_brand_node"],
            sc_types.EDGE_ACCESS_VAR_POS_PERM,
            self._keynodes["nrel_operator"]
        )
        ...
        for item in result:
            _logger.debug("SearchBanksByBrandAgent get answer: " +
