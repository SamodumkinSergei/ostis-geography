...
SC_AGENT_IMPLEMENTATION(SchoolSearchAgent)
	{
		...
		ScAddr school = IteratorUtils::getFirstByOutRelation(
			&m_memoryCtx,
			actionNode,
		...
		ScIterator5Ptr it1 = ms_context->Iterator5(ScType::Unknown, ScType::EdgeDCommonConst, school, ScType::EdgeAccessConstPosPerm, Keynodes::nrel_school_number);
		ScAddr school1;
		ScAddr schoolResult;
		if (it1->Next())
		{
			school = it1->Get(0);
			ScIterator5Ptr it2 = ms_context->Iterator5(school, ScType::EdgeDCommonConst, ScType::Unknown, ScType::EdgeAccessConstPosPerm, Keynodes::nrel_search_area);
			if (it2->Next())
			{
