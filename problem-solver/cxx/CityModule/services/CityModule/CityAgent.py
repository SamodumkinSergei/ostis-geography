    ...
    def RunImpl(self, evt: ScEventParams) -> ScResult:
        ...
        if self.module.ctx.HelperCheckEdge(
            ...
            try:
                ...
                definitionIterator = self.ctx.Iterator5(  # находит всё что есть в городе
                    ScType.Unknown,
                    ScType.EdgeDCommon,
                    node,
                    ScType.EdgeAccessConstPosPerm,
                ...
                while (definitionIterator.Next()):
                    townElement = definitionIterator.Get(0)  # это town element
                    pashaIterator = self.ctx.Iterator3(  # находит всё что есть в городе
                        self.keynodes['concept_monument'],
                        ScType.EdgeAccessConstPosPerm,
                        townElement,
                    )
                    if pashaIterator.Next():
                        self.add_nodes_to_answer(answerNode,
                                                 [townElement, definitionIterator.Get(1), definitionIterator.Get(2)])
