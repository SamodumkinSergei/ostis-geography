...
namespace dance_studios {
  ...
  SC_AGENT_IMPLEMENTATION(SearchDanceStudiosByFoundingYearAgent)
  {
    ...
    std::unique_ptr<DanceStudiosByPropertyInNumericalRangeFinder>
        danceStudiosByPropertyInNumericalRangeFinder =
        std::make_unique<DanceStudiosByPropertyInNumericalRangeFinder>();

    ScAddr questionNode = ms_context->GetEdgeTarget(edgeAddr);
    ScAddr answer = danceStudiosByPropertyInNumericalRangeFinder->
        findDanceStudiosByPropertyInNumericalRange(
        ms_context.get(),
        questionNode,
