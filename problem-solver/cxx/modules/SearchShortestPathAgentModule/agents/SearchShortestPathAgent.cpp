...
#include "SearchShortestPathAgent.hpp"
...
SC_AGENT_IMPLEMENTATION(SearchShortestPathAgent)
{
  if (!edgeAddr.IsValid())
  {
  ...
  SC_LOG_INFO("SearchShortestPathAgent begin");
...
Node* SearchShortestPathAgent::getNodeFromNodesVectorByScAddr(const ScAddr& scAddr, std::vector<Node*>& nodesVector) {
  for (int i = 0; i < nodesVector.size(); i++)
  {
...
std::vector<ScAddr> SearchShortestPathAgent::getConnectedCrossroads(ScMemoryContext * ms_context, const ScAddr & node)
{
...
int SearchShortestPathAgent::getDistanceBetweenNodes(
...
void SearchShortestPathAgent::setMinDistanceTags(ScMemoryContext * ms_context, Node * startNode)
{
...
void SearchShortestPathAgent::getMinPath(
    ScMemoryContext * ms_context,
    std::vector<Node *> & resultNodesVector,
    Node * currentNode)
{
