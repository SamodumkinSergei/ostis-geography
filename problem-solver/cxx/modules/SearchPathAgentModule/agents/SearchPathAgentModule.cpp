...
SC_AGENT_IMPLEMENTATION(SearchPathAgent)
{
  if (!edgeAddr.IsValid())
    return SC_RESULT_ERROR;

  SC_LOG_INFO("SearchPathAgent begin");
...
void SearchPathAgent::findMinimalPath(
    ScAddr currentNode,
    ScAddr target,
    int currentLength,
    int& resultLength,
    ...
    std::vector<ScAddr>& result)
{
  ...
  for (auto incidentNode : incidentNodes)
  {
    ...
    findtimepath(incidentNode, target, currentLength + minutesCount, resultLength, currentPath, result);
  }

  currentPath.pop_back();
}

std::list<ScAddr> SearchPathAgent::getAllIncidentNodes(const ScAddr& node)
{
...
int SearchPathAgent::getMinutesCount(ScAddr edge)
{
  auto minutesNode = getMinutesNode(edge);
...
ScAddr SearchPathAgent::getMinutesNode(const ScAddr& edge)
{
...
ScAddr SearchPathAgent::getEdgeBetween(const ScAddr& from, const ScAddr& to, ScType type, bool findReverse)
{
