...
#include "FindingTimeToGetToMinskAgent.hpp"
...
SC_AGENT_IMPLEMENTATION(FindingTimeToGetToMinskAgent)
{
  if (!edgeAddr.IsValid())
    return SC_RESULT_ERROR;

  SC_LOG_INFO("FindingTimeToGetToMinskAgent begin");
...

  {
    ...
    findTime(startNodeAddr, endNodeAddr, 0, length, currentPath, result);
  }
  ...
  SC_LOG_INFO("FindingTimeToGetToMinskAgent end");
  return SC_RESULT_OK;
}

void FindingTimeToGetToMinskAgent::findTime(
    ScAddr currentNode,
    ScAddr target,
    int currentLength,
    int& resultLength,
    ...
    std::vector<ScAddr>& result)
{
  ...
  for (auto incidentNode : incidentNodes)
  {
    ...
    findTime(incidentNode, target, currentLength + timeCost, resultLength, currentPath, result);
  }
...
std::list<ScAddr> FindingTimeToGetToMinskAgent::getIdentifierMeta(const ScAddr& addr)
{
...
int FindingTimeToGetToMinskAgent::getTimeCost(ScAddr edge)
{
  auto timeNode = getTimeNode(edge);
...
ScAddr FindingTimeToGetToMinskAgent::getTimeNode(const ScAddr& edge)
{
...
ScAddr FindingTimeToGetToMinskAgent::getEdgeBetween(const ScAddr& from, const ScAddr& to, ScType type, bool findReverse)
{
