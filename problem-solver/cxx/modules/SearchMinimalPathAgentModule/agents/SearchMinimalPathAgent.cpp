...
#include "SearchMinimalPathAgent.hpp"
...
SC_AGENT_IMPLEMENTATION(SearchMinimalPathAgent)
{
  if (!edgeAddr.IsValid())
    return SC_RESULT_ERROR;

  SC_LOG_INFO("SearchMinimalPathAgent begin");
  ...
  for (int i = 1; i < result.size(); ++i)
  {
    ...
    auto kmNode = getKmNode(edge);

    auto edgeToKmNode = getEdgeBetween(edge, kmNode, ScType::EdgeAccessConstPosPerm, false);
    auto kmNodeMeta = getIdentifierMeta(kmNode);
    ...
    ms_context->CreateEdge(ScType::EdgeAccessConstPosPerm, answer, kmNode);
    ms_context->CreateEdge(ScType::EdgeAccessConstPosPerm, answer, edgeToKmNode);

    for (auto item : kmNodeMeta)
    {
      ms_context->CreateEdge(ScType::EdgeAccessConstPosPerm, answer, item);
    }
  }
  ...
  SC_LOG_INFO("SearchMinimalPathAgent end");
  return SC_RESULT_OK;
}

void SearchMinimalPathAgent::findMinimalPath(
    ScAddr currentNode,
    ScAddr target,
    int currentLength,
    int& resultLength,
    ...
    std::vector<ScAddr>& result)
{
  ...
  for (auto incidentNode : incidentNodes)
  {
    ...
    auto kmCount = getKmCount(edge);

    findMinimalPath(incidentNode, target, currentLength + kmCount, resultLength, currentPath, result);
  }

  currentPath.pop_back();
}

std::list<ScAddr> SearchMinimalPathAgent::getAllIncidentNodes(const ScAddr& node)
{
...
std::list<ScAddr> SearchMinimalPathAgent::getIdentifierMeta(const ScAddr& addr)
{
...
int SearchMinimalPathAgent::getKmCount(ScAddr edge)
{
  auto kmNode = getKmNode(edge);
  auto idtf = CommonUtils::getMainIdtf(ms_context.get(), kmNode);

  return std::stoi(idtf);
}

ScAddr SearchMinimalPathAgent::getKmNode(const ScAddr& edge)
{
  auto iterator = ms_context->Iterator3(ScType::NodeConst, ScType::EdgeAccessConstPosPerm, edge);

  while (iterator->Next())
  {
    auto kmNode = iterator->Get(0);

    if (!ms_context->HelperCheckEdge(Keynodes::concept_parameter, kmNode, ScType::EdgeAccessConstPosPerm))
    {
      continue;
    }
...
ScAddr SearchMinimalPathAgent::getEdgeBetween(const ScAddr& from, const ScAddr& to, ScType type, bool findReverse)
{
