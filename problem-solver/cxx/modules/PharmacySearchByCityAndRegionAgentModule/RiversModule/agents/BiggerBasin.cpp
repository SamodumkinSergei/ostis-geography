...
ScAddr BiggerBasin::GetActionClass() const
{
  return RiverKeynodes::action_get_biggerBasin;
}


ScResult BiggerBasin::DoProgram(ScAction & action)
{
  ...
  ScIterator5Ptr it = m_context.CreateIterator5(
      river1, ScType::ConstCommonArc, ScType::Unknown, ScType::ConstPermPosArc, RiverKeynodes::nrel_basin);
  int l1 = 0;
  while (it->Next())
  {
    ScAddr len = it->Get(2);
  ...
  ScIterator5Ptr it1 = m_context.CreateIterator5(
      river2, ScType::ConstCommonArc, ScType::Unknown, ScType::ConstPermPosArc, RiverKeynodes::nrel_basin);
  int l2 = 0;
  while (it1->Next())
  {
  ...
  if (l1 > l2)
  {
    ScIterator5Ptr iteratorToAddToAnswer = m_context.CreateIterator5(
        river1, ScType::Unknown, ScType::Unknown, ScType::ConstPermPosArc, RiverKeynodes::nrel_basin);
  ...
  {
    ScIterator5Ptr iteratorToAddToAnswer = m_context.CreateIterator5(
        river2, ScType::Unknown, ScType::Unknown, ScType::ConstPermPosArc, RiverKeynodes::nrel_basin);
