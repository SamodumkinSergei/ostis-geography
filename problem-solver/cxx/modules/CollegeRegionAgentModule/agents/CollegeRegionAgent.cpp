...
#include "CollegeRegionAgent.hpp"
...
namespace CollegeRegionAgentModule
{
    ...
    SC_AGENT_IMPLEMENTATION(CollegeRegionAgent)
    {
        if (!edgeAddr.IsValid())
            return SC_RESULT_ERROR;

        SC_LOG_INFO("CollegeRegionAgent begin");
        ...
        SC_LOG_INFO("CollegeRegionAgent end");
        return SC_RESULT_OK;
    }

    std::list<ScAddr> CollegeRegionAgent::getIdentifierMeta(const ScAddr& addr)
    {
    ...
    ScAddr CollegeRegionAgent::getStatusNode(const ScAddr& edge)
    {
    ...
    ScAddr CollegeRegionAgent::getEdgeBetween(const ScAddr& from, const ScAddr& to, ScType type, bool findReverse)
    {
