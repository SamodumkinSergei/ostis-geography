...
#include "CityBelongsSearchAgent.hpp"
...
namespace CityBelongsSearchAgentModule
{
    ...
    SC_AGENT_IMPLEMENTATION(CityBelongsSearchAgent) {
        if (!edgeAddr.IsValid()) {
            return SC_RESULT_ERROR;
        }

        SC_LOG_INFO("CityBelongsSearchAgent begin");
        ...
        SC_LOG_INFO("CityBelongsSearchAgent end");
        return SC_RESULT_OK;
    }

    std::list<ScAddr> CityBelongsSearchAgent::getIdentifierMeta(const ScAddr& addr)
    {
    ...
    ScAddr CityBelongsSearchAgent::getStatusNode(const ScAddr& edge) {
    ...
    ScAddr CityBelongsSearchAgent::getEdgeBetween(const ScAddr& from, const ScAddr& to, ScType type, bool findReverse) {
