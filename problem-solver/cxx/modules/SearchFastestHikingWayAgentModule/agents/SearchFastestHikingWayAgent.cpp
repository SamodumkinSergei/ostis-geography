...
#include "SearchFastestHikingWayAgent.hpp"
...
SC_AGENT_IMPLEMENTATION(SearchFastestHikingWayAgent)
{
  if (!edgeAddr.IsValid())
    return SC_RESULT_ERROR;

  SC_LOG_INFO("SearchFastestHikingWayAgent begin");
  ...
  SC_LOG_INFO("SearchFastestHikingWayAgent end");
  return SC_RESULT_OK;
}

void SearchFastestHikingWayAgent::findMinimalPath(
    ScAddr currentNode,
    ScAddr target,
    int currentLength,
    int& resultLength,
...
std::list<ScAddr> SearchFastestHikingWayAgent::getAllIncidentNodes(const ScAddr& node)
{
...
std::list<ScAddr> SearchFastestHikingWayAgent::getIdentifierMeta(const ScAddr& addr)
{
...
int SearchFastestHikingWayAgent::getMinutesCount(ScAddr edge)
{
  auto minutesNode = getMinutesNode(edge);
...
ScAddr SearchFastestHikingWayAgent::getMinutesNode(const ScAddr& edge)
{
...
ScAddr SearchFastestHikingWayAgent::getEdgeBetween(const ScAddr& from, const ScAddr& to, ScType type, bool findReverse)
{
