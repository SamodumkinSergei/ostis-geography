...
SC_AGENT_IMPLEMENTATION(SearchMinPathAgent)
{
  ...
  SC_LOG_INFO("Start: SearchMinPathAgent");

  ScAddr actionNode = ms_context->GetEdgeTarget(edgeAddr);

  const ScAddr & startNodeAddr = IteratorUtils::getFirstByOutRelation(&m_memoryCtx, actionNode, scAgentsCommon::CoreKeynodes::rrel_1);

  const ScAddr& endNodeAddr = IteratorUtils::getFirstByOutRelation(&m_memoryCtx, actionNode, scAgentsCommon::CoreKeynodes::rrel_2);
  
  if (!startNodeAddr.IsValid() ||!endNodeAddr.IsValid())
  { 
    SC_LOG_ERROR("Nodes are not valid.");
  ...
  {
    auto iter5ArrayOfCrossRoads = ms_context->Iterator5(iter3Road->Get(2), ScType::EdgeDCommonConst, ScType::NodeConst, ScType::EdgeAccessConstPosPerm, Keynodes::nrel_bordering_points);

    while (iter5ArrayOfCrossRoads->Next())
    {
      auto iter3NodeAddr = ms_context->Iterator3(iter5ArrayOfCrossRoads->Get(2), ScType::EdgeAccessConstPosPerm, ScType::NodeConst);
  ...
  for (int i = 0; i < nodesVector.size(); i++)
  {
    if (nodesVector[i]->addrNode == startNodeAddr)
    {
      firstNode = nodesVector[i];
      break;
    }
  }
  ...
  for (int i = 0; i < nodesVector.size(); i++)
  {
    if (nodesVector[i]->addrNode == endNodeAddr)
    {
      lastNode = nodesVector[i];
      break;
    }
  }
  ...
  for(int i = 0, j = resultNodesVector.size(); i < resultNodesVector.size(); i++, j--) {
    ...
    auto iter5NodeIdth = ms_context->Iterator5(resultNodesVector[i]->addrNode, ScType::EdgeDCommonConst, ScType::Unknown, ScType::EdgeAccessConstPosPerm, Keynodes::nrel_main_idtf);
  ...
  SC_LOG_INFO("End: SearchMinPathAgent");
  return SC_RESULT_OK;
}
...
std::vector<ScAddr> SearchMinPathAgent::getConnectedCrossroads(ScMemoryContext * ms_context, const ScAddr & node)
{
  ...
  {
    auto iter5ArraysOfCrossroads = ms_context->Iterator5( ScType::NodeConst, ScType::EdgeDCommonConst, iter3ArraysOfCrossroads->Get(0), ScType::EdgeAccessConstPosPerm, Keynodes::nrel_bordering_points);
    while (iter5ArraysOfCrossroads->Next())
    {
      auto iter3crossroads = ms_context->Iterator3(iter5ArraysOfCrossroads->Get(2), ScType::EdgeAccessConstPosPerm, ScType::NodeConst);
    ...
    const ScAddr & secondNode)
{
  ...
  auto iter5Road = ms_context->Iterator5(ScType::NodeConst, ScType::EdgeDCommonConst, arrayOfCrossRoads, ScType::EdgeAccessConstPosPerm, Keynodes::nrel_bordering_points);
  while (iter5Road->Next())
  {
...
void SearchMinPathAgent::setMinDistanceTags(ScMemoryContext * ms_context, Node * startNode)
{
  ...
  while (!queue.empty())
  {
    ...
    for (int i = 0; i < currentNode->connectedNodes.size(); i++)
    {
      ...
      {
        auto distanceTag = currentNode->minDistanceTag + getDistanceBetweenNodes(ms_context, currentNode->addrNode, currentNode->connectedNodes[i]->addrNode);
        currentNode->connectedNodes[i]->minDistanceTag = std::min(distanceTag, currentNode->connectedNodes[i]->minDistanceTag);
      }
    }
    currentNode->isVisited = true;
    std::sort(currentNode->connectedNodes.begin(), currentNode->connectedNodes.end(), [](const Node * lhs, const Node * rhs) 
    {
...
void SearchMinPathAgent::getMinPath(ScMemoryContext * ms_context, std::vector<Node *> & resultNodesVector, Node * currentNode)
{
